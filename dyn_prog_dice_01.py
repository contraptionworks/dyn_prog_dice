...
# The main_loop starts with an array of possible outcomes from one die
    # and calls the add_base_die function in a loop until it reaches the number of dice chosen.
# The script operates in raw combinations, so each die added multiplies the total by six.
    # Percentages are only calculated for output.

print("            0     1     2     3")
print("          Miss  Weak  Strong Crit")

def add_base_die(input):
    # Input is an array of dice result combinations from the main_loop,
        # which increments from one base die to the number chosen in the last line.
    # Each possible result on the added die has various effects on elements in the input array.
    ...
    # Continue result 6: Any input strong hits become crits, and keep any input crits
